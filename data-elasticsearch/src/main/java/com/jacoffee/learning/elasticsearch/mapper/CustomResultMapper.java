package com.jacoffee.learning.elasticsearch.mapper;

import com.fasterxml.jackson.core.JsonEncoding;
import com.fasterxml.jackson.core.JsonFactory;
import com.fasterxml.jackson.core.JsonGenerator;
import com.jacoffee.learning.elasticsearch.annotations.SortValues;
...
public class CustomResultMapper extends AbstractResultMapper {
    ...
    public <T> AggregatedPage<T> mapResults(SearchResponse response, Class<T> clazz, Pageable pageable) {
        ...
        for (SearchHit hit : response.getHits()) {
            if (hit != null) {
                ...
                // 设置SortValues
                setEntitySort(result, hit.getSortValues(), clazz);
    ...
    /**
     * 由于当前版本不支持sort对应字段实现，也没有提供覆盖接口所以复制了DefaultResultMapper然后直接修改
     * @param result result
     * @param sortValues sortValues
     * @param clazz clazz
     * @param <T> t
     */
    private <T> void setEntitySort(T result, Object[] sortValues, Class<T> clazz) {
        if (clazz.isAnnotationPresent(Document.class)) {
            ElasticsearchPersistentEntity<?> persistentEntity = mappingContext.getPersistentEntity(clazz);
            ElasticsearchPersistentProperty sortValuesProperty =
                persistentEntity.getPersistentProperty(SortValues.class);

            if (
                Objects.nonNull(sortValuesProperty) &&
                    Objects.nonNull(sortValues) &&
                    sortValues.length > 0
            ) {
                persistentEntity.getPropertyAccessor(result).setProperty(sortValuesProperty, sortValues);
            }
        }
    }
